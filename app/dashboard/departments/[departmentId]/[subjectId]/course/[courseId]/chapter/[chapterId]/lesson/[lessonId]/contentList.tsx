...
const ContentList:React.FC<ContentListProps> = ({lesson}) => {
  ...
  return (<div className='flex flex-col gap-4'>
   {lesson?.contents?.map((content:any) =>{
    return  <div  key={content?.id} className="p-4 relative w-full bg-white dark:bg-gray-800 hover:border-2 hover:border-sky-400 hover:border-dashed  transition duration-300 group" dangerouslySetInnerHTML={{__html:content?.content}}>
