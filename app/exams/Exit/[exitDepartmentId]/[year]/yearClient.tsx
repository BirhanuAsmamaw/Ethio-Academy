...
const ExitYearExamsClientPage:React.FC<ExitYearExamsClientPageProps> = ({department,year,Questions}) => {
  ...
  if (!Questions || !Questions.length){
    return (<div className="h-screen  p-4 w-full flex justify-center items-center">
      <NoExamNotification notification={`There are No Exit Exams in ${year} Year`} url={`/exams/Exit/${department.id}`} label={`Click Here and See ${department.departmentName} Exit Exams in Others Years`}/>
    </div>)
  }
  return (<MainLayout>
