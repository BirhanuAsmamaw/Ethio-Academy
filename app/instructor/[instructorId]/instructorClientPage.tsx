...
import { useGetInstructorQuery } from '@/redux/features/instructors/instructorApi';
const eb_garamound= EB_Garamond({ subsets: ['latin'] ,weight:['400', '500','600','700']})
const InstructorClientPage = ({instructorId}:{instructorId:string}) => {
  const {data:teacher,isSuccess,isLoading,isError}=useGetInstructorQuery(instructorId)
...
/>
       ...
       {isSuccess?<div className="flex py-20 justify-center">
...
<SubscriberAccount accountId={teacher?.id||""} is_subscriber={teacher?.is_subscriber||false} user={teacher?.isUser}/>
</div>
...
export default InstructorClientPage