...
{subjectCat?    <div className="p-2 absolute bottom-0 -left-4 opacity-0 z-10   group-hover:left-0 bg-white dark:bg-gray-800 leading-5 group-hover:opacity-100 text-sm font-medium shadow-md drop-shadow-md text-gray-600 dark:text-green-400 bg-opacity-80 dark:bg-opacity-80 flex justify-center transition-all duration-300">
  ...
  <div className="flex flex-col  pt-2 gap-1 md:p-2 md:gap-2 w-full">
    ...
    <div className="flex justify-center w-full">
      <h1 className=" w-full hidden md:block text-base md:text-lg tracking-tight  text-center  font-medium md:font-semibold p-1">{subject.length>21? subject.substring(0,19)+"...":subject}</h1>
      <h1 className=" md:hidden w-full text-base md:text-lg  tracking-tight  text-center font-medium md:font-semibold p-1">{subject.length>30? subject.substring(0,30)+"...":subject}</h1>
    </div>
    ...
    {instructorName?<Link href={`/instructor/${instructorId}`} className="hover:underline no-underline transition px-1 md:px-0 md:py-2 flex gap-1 md:gap-2">
      ...
      {instructorName?<div className="">
        <div className=" text-sm md:text-[14px]  font-medium text-gray-900 dark:text-gray-200  tracking-tight flex gap-2"><p>{instructorName}</p> <ChannelVerfiedComponent/></div>
  {instructorTitle?      <p className="text-gray-500 dark:text-gray-400 text-xs  tracking-tight">{instructorTitle}</p>:""}
      </div>:""}
    </Link>:""}
